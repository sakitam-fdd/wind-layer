...
const Windy = function (params = {}) {
  ...
  // interpolation for vectors like wind (u,v,m)：TODO: 已转义
...
class Field {
   ...
   * i.e., floor(a / n). Useful for consistent modulo
   * of negative numbers. See http://en.wikipedia.org/wiki/Modulo_operation.
   */
  floorMod (a, n) {
    return a - n * Math.floor(a / n);
   ...
   * vector is modified in place and returned by this function.
   */
  distort (projection, λ, φ, x, y, scale, wind, windy) {
    var u = wind[0] * scale;
    var v = wind[1] * scale;
  ...
  distortion (projection, λ, φ, x, y, windy) {
    var τ = 2 * Math.PI;
    // var H = Math.pow(10, -5.2);
    var H = that.params.projection === 'EPSG:4326' ? 5 : Math.pow(10, -5.2);
    var hλ = λ < 0 ? H : -H;
  ...
  createWindBuilder (uComp, vComp) {
    ...
    return {
      ...
      interpolate: that.bilinearInterpolateVector
    }
  }
...
class WindLayer {
  ...
  start(bounds, width, height, extent) {
    ...
    // build grid
    buildGrid(gridData) {
      const field = new Field(gridData);
      // interpolateField
      interpolateField(grid, buildBounds(bounds, width, height), mapBounds, function (bounds, field) {
        // animate the canvas with random points
        windy.field = field;
