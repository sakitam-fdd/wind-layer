...
//    vec2 uv = (pos - u_data_bbox.xy) / (u_data_bbox.zw - u_data_bbox.xy);
    ...
    if (pos.xy == drop_pos.xy) {
        float drop_rate = u_drop_rate + speed_t * u_drop_rate_bump;
        float drop = step(1.0 - drop_rate, rand(seed));
        vec2 random_pos = vec2(rand(seed + 1.3), rand(seed + 2.1));

//        return mix(u_bbox.xy, u_bbox.zw, random_pos);
    }

    // 源位置不在数据范围内，置为初始位置
    if (!containsXY(pos, u_data_bbox)) {
//        return drop_pos;
    }

    // 源位置不在地图范围内，置为初始位置
    if (!containsXY(pos, u_bbox)) {
        return drop_pos;
    }

    // update particle position, wrapping around the date line
    pos = fract(1.0 + pos + offset);
    seed = (pos + v_tex_pos) * u_rand_seed;
